-- ═══════════════════════════════════════════════════════════════════════════
-- 42. FIX URGENTE: Habilitar pgcrypto + Recrear login_seguro
-- ERROR: "function crypt(text, text) does not exist"
-- CAUSA: pgcrypto no instalado o search_path no lo encuentra
...
-- 3. Recrear login_seguro con search_path que incluya extensions
...
LANGUAGE plpgsql
SECURITY DEFINER
-- CRÍTICO: incluir 'extensions' en el search_path para que crypt() funcione
...
SELECT public.login_seguro('marketcoffee', 'market.2026', '127.0.0.1', 'test');
